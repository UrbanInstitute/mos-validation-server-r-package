...
This R package lets users submit analyses to Urban Institute's validation server. A valid R script must: 
...
* `stat` is the summary statistic (or set of summary statistics) to calculate. This can include `mean`, `median`, `sum`, `sd`, `var`, `min`, `max`, `n` or `n_distinct`. 
* `var` is the variable (or set of variables) to summarize. 
* `by` (optional) is the variable (or set of variables) to group by. 
* `na.rm` (optional) specifies explicit NA behavior (defaults to `TRUE`). 
