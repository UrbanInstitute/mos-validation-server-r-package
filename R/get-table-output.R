...
final_output <- bind_rows(output, output_quantile) %>% # bind non-quantile and quantile summary stats together
    ...
    dplyr::mutate(statistic = dplyr::if_else(stringr::str_detect(statistic, "quantile"),
                                      stringr::str_c("quantile_",
                                                     stringr::str_extract(statistic, "[0-9]+") %>%
                                                         as.numeric()
                                                     ), statistic)) # we originally create quantile variable with padded 3 digits for sorting purposes
                                                                    # this code rewrites the format back to `quantile_[0-100]`


# if we grouped by specific variables, we combine them into one group column
if(length(by) > 0){
final_output <- final_output %>%
    dplyr::mutate(dplyr::across(all_of(by), ~paste0(cur_column(), " = ", .))) %>%
    tidyr::unite(group, all_of(by), sep = ", ")
}
...
# the quantile function throws an error if any NAs exist. this code will instead return NA if na.rm is false and there are missing values.
